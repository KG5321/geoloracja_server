...
1 ul. T.Kosciuszki 135 51.099 N, 17.046 E
2 ul. Wittiga 8 - dachu domu studenckiego T-19 51.102 N, 17.085 E
3 ul. Reja 54-56, dach akademika T-6 51.118 N, 17.058 E
"""
def create_dataset(fname, area):
...
def trainRandomForrest(trainingDataset, targetDataset, numberOfEstimators = 10):
...
def dumpClassifier(classifier, fname):
    fp = gzip.open('classifier.pickle.gzip','wb') # This assumes that primes.data is already packed with gzip
    cPickle.dump(classifier, fp)
    fp.close()
 
def loadClassifier(fname):
    fp = gzip.open('classifier.pickle.gzip','rb') # This assumes that primes.data is already packed with gzip
...
if __name__ == "__main__":
    ...
    X, y = create_dataset(dataset_fname, sample_area)
    
    print "creating classifier"
    
    classifier, name = trainRandomForrest(np.array(X), np.array(y))
